...
export default function BollingerWidget() {
  // 1. Initialize with an empty array to ensure server and client match exactly on first render
  const [prices, setPrices] = useState<number[]>([]);
  const [windowSize] = useState<number>(5); 
  const [multiplier, setMultiplier] = useState<number>(2.0);

  // 2. Generate the simulated data only after mounting on the browser client
  useEffect(() => {
  ...
  // 3. Provide fallback geometry boundaries for when data array is empty during SSR
      ...
      <div className="grid gap-6 lg:grid-cols-3 items-start">
        <div className="lg:col-span-2 rounded-2xl border border-slate-900 bg-slate-950/20 p-4 space-y-4">
          ...
          <div className="relative w-full h-56 bg-slate-950/80 rounded-xl overflow-hidden border border-slate-900">
            {/* 4. Only render the inner shapes once data points are ready on the client */}
